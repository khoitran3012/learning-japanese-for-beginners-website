import { n as getPglite, t as ensureDbReady } from "./db-iW6MThKt.mjs";
import { n as binary, t as createHMAC } from "../_libs/better-auth__utils.mjs";
import { n as verifyPassword$1, t as hashPassword$1 } from "./password-VlpK0Xix.mjs";
import { $t as queueAfterTransactionHook, A as email, C as serializeSignedCookie, D as any, F as optional, Gt as withSpan, I as record, J as decodeProtectedHeader, Jt as ATTR_OPERATION_ID, K as base64Url, Kt as ATTR_CONTEXT, L as string, M as looseObject, N as number, O as array, P as object, Qt as getCurrentAdapter, S as serializeCookie, V as betterFetch, W as normalizePathname, X as jwtVerify, Xt as safeJSONParse, Y as importJWK, Yt as import_src, Zt as getAuthTables, _ as runWithRequestState, _n as APIError, a as createAuthorizationURL, an as generateId, b as createRouter$1, bn as BASE_ERROR_CODES, c as createRateLimitKey, cn as logger, d as deprecate, dn as env, en as runWithAdapter, f as createAuthEndpoint, g as hasRequestState, gn as isTest, h as defineRequestState, hn as isProduction, i as refreshAccessToken, in as initGetFieldName, jt as JWTExpired, k as boolean, l as findInvalidTrustedProxies, ln as shouldPublishLog, m as isAPIError, mn as isDevelopment, n as socialProviders, nn as getBetterAuthVersion, o as applyDefaultAccessTokenExpiry, on as createRandomStringGenerator, p as createAuthMiddleware, q as decodeJwt, qt as ATTR_HOOK_TYPE, r as validateAuthorizationCode, rn as initGetModelName, s as isLoopbackHost, sn as createLogger, t as SocialProviderListEnum, tn as runWithTransaction, u as getIp, v as getCurrentAuthContext, vn as BetterAuthError, w as filterOutputFields, x as toResponse, y as runWithEndpointContext, yn as kAPIErrorHeaderSymbol, yt as encode } from "../_libs/@better-auth/core+[...].mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/server-Dk6lnkaw.js
...
var SELF_HOST_HOSTS = ["khoitran3012.ddns.net", "*.ddns.net"];
...
function originsForHost(host) {
	...
	for (const proto of ["http", "https"]) for (const port of ports) out.push(`${proto}://${host}${port}`);
	return out;
}
function extraPublicOrigins() {
	...
	add(explicitBaseURL);
	add(env$1("AKARI_PUBLIC_ORIGIN"));
	add(env$1("BETTER_AUTH_TRUSTED_ORIGINS"));
	for (const host of ["khoitran3012.ddns.net"]) for (const origin of originsForHost(host)) out.add(origin);
	return [...out];
}
var extraOrigins = extraPublicOrigins();
...
var staticTrustedOrigins = explicitBaseURL ? [
	explicitBaseURL,
	...LOCAL_DEV_ORIGINS,
	...extraOrigins
] : [
...
function isSelfHostOrigin(origin) {
	try {
		const host = new URL(origin).hostname;
		return host === "khoitran3012.ddns.net" || host.endsWith(".ddns.net") || host.endsWith(".grok-sandbox.com");
	} catch {
		return false;
	}
}
/** Same-origin: Origin/Referer host matches this request's Host (any public DDNS/LAN name). */
function originMatchesRequestHost(origin, request) {
	try {
		const parsed = new URL(origin);
		const reqHost = (request.headers.get("x-forwarded-host") || request.headers.get("host") || "").split(",")[0]?.trim().toLowerCase();
...
var trustedOrigins = async (request) => {
	...
	if (request) {
		...
		if (header && header !== "null") try {
			const origin = header.startsWith("http") ? new URL(header).origin : header;
			if (isSelfHostOrigin(origin) || originMatchesRequestHost(origin, request)) list.push(origin);
		} catch {}
	}
	return [...new Set(list.filter(Boolean))];
};
function useInsecureCookies() {
	if (env$1("AKARI_INSECURE_COOKIES") === "1" || env$1("AKARI_SELF_HOST") === "1") return true;
	const publicUrl = env$1("BETTER_AUTH_URL") || env$1("AKARI_PUBLIC_ORIGIN");
...
var insecureCookies = useInsecureCookies();
...
var SESSION_TOKEN_COOKIE = insecureCookies ? "akari-auth.session_token" : "__Host-grok-auth.session_token";
...
//#endregion
export { gateIdentityEnabled as a, authConfigured as i, SESSION_TOKEN_COOKIE as n, hasGateSessionMarker as o, auth as r, GROK_PROVIDERS as t };
