...
//#region node_modules/.nitro/vite/services/ssr/assets/progress-BOD7PV7B.js
...
var useProgress = create((set, get) => ({
	...
	logStudy: async (items = 1, minutes = .5) => {
		...
		const snap = get();
		import("./sync-C0jeQzAT.mjs").then((n) => n.n).then(({ scheduleGardenSync }) => scheduleGardenSync({
			srs: snap.srs,
	...
	completeLesson: async (id) => {
		...
		import("./sync-path-Bay3TKYM.mjs").then(({ syncPathProgress }) => syncPathProgress([...get().completedLessonIds]));
