...
import { t as GROK_PROVIDERS } from "./server-DxKUXtG1.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/login-ojbQiMpx.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var import_jsx_runtime = require_jsx_runtime();
function humanAuthError(message, fallback) {
	const raw = (message || "").toLowerCase();
	if (raw.includes("invalid origin") || raw.includes("unable to verify") || raw.includes("forbidden")) return "Địa chỉ web này chưa được tin cậy cho đăng nhập. Hãy mở đúng địa chỉ của ứng dụng rồi thử lại.";
...
function Login() {
	...
	async function onEmail(e) {
		...
		try {
			...
			setError("Không đăng nhập được. Nếu bạn mở web từ tên miền riêng, hãy dùng đúng địa chỉ máy chủ và cho phép cookie.");
		} finally {
			setBusy(false);
		}
	}
