import { r as kanjiLessonByPath } from "./kanji-lessons-Bonc0FIc.mjs";
import { n as KATAKANA, t as HIRAGANA } from "./kana-CV-aAiLY.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/lesson-links-DiU0qHPI.js
var ROW_LESSON = {
	"l-n5-hira-a": "a",
	"l-n5-hira-ka": "ka",
	"l-n5-hira-sa": "sa",
...
function stageLink(lesson) {
	...
	switch (lesson.stage) {
		...
		case "giao tiếp":
			if (lesson.focus === "nghe nói") return {
				to: "/listen",
				label: "Luyện nghe"
			};
			return {
		...
		case "kanji": {
			...
			if (kl) return {
				to: `/kanji?lesson=${kl.id}`,
				label: `Kanji: ${kl.title}`
			};
			return {
				to: "/kanji",
				label: "Kanji"
...
function lessonPracticeLinks(lesson) {
	...
	if (kanjiIds.length > 4) {
		...
		if (!seen.has(href)) {
			...
			links.push({
				to: href,
				label: `${kanjiIds.length} chữ kanji`
			});
		}
	}
