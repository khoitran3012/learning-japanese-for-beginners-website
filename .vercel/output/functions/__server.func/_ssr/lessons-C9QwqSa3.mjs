import { t as KANJI_LESSONS } from "./kanji-lessons-Bonc0FIc.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/lessons-C9QwqSa3.js
...
var existing = new Set(PATH_LESSONS.map((l) => l.id));
function practiceIds(level, chars) {
	const lv = level.toLowerCase();
	return [...chars].map((ch) => `kj-${lv}-${ch}`);
}
/** Bài kanji N5–N4 chưa viết tay — vẫn nằm trên lộ trình học. */
function extraKanjiPathLessons() {
	...
	return extras.map((k) => {
		...
		const order = n5 ? 27.5 + idx * .1 : 35 + (idx + 1) * .1;
		return {
			id: k.pathLessonId,
			title: `Kanji: ${k.title}`,
			title_jp: k.title_jp,
			...
			sections: [{
				...
				body: n5 ? `Bài này có ${k.chars.length} chữ N5: ${[...k.chars].join(" ")}. Nhìn chữ, nói Hán-Việt, viết nét, rồi mới nghe kun/on.` : `Bài này có ${k.chars.length} chữ N4: ${[...k.chars].join(" ")}. Thử on trong từ ghép hai chữ trước khi kun.`,
