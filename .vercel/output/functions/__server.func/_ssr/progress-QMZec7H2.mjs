...
//#region node_modules/.nitro/vite/services/ssr/assets/progress-QMZec7H2.js
...
var useProgress = create((set, get) => ({
	...
	logStudy: async (items = 1, minutes = .5) => {
		...
		const snap = get();
		import("./sync-Dfros0w-.mjs").then((n) => n.n).then(({ scheduleGardenSync }) => scheduleGardenSync({
			srs: snap.srs,
	...
	completeLesson: async (id) => {
		...
		import("./sync-path-BEN9NbZT.mjs").then(({ syncPathProgress }) => syncPathProgress([...get().completedLessonIds]));
