...
//#region node_modules/.nitro/vite/services/ssr/assets/progress-DKGo45g_.js
...
var useProgress = create((set, get) => ({
	...
	logStudy: async (items = 1, minutes = .5) => {
		...
		const snap = get();
		import("./sync-BD1lbPGL.mjs").then((n) => n.n).then(({ scheduleGardenSync }) => scheduleGardenSync({
			srs: snap.srs,
	...
	completeLesson: async (id) => {
		...
		import("./sync-path-BihSW368.mjs").then(({ syncPathProgress }) => syncPathProgress([...get().completedLessonIds]));
