...
//#region node_modules/.nitro/vite/services/ssr/assets/progress-BQrBv4Tb.js
...
var useProgress = create((set, get) => ({
	...
	logStudy: async (items = 1, minutes = .5) => {
		...
		const snap = get();
		import("./sync-BhlkWSdG.mjs").then((n) => n.n).then(({ scheduleGardenSync }) => scheduleGardenSync({
			srs: snap.srs,
	...
	completeLesson: async (id) => {
		...
		import("./sync-path-CcT25a2w.mjs").then(({ syncPathProgress }) => syncPathProgress([...get().completedLessonIds]));
