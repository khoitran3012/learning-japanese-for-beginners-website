...
//#region node_modules/.nitro/vite/services/ssr/assets/sync-path-CcT25a2w.js
...
var recordPathProgress = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((input) => ({ lessonIds: Array.from(new Set((Array.isArray(input?.lessonIds) ? input.lessonIds : []).map((id) => String(id)))).filter((id) => Boolean(LESSON_STAGE[id])).slice(0, 80) })).handler(createSsrRpc("ea01e3626d50871b0e27b68fe2793e1efa4c516b9a72c31078cd03fd5eb6d5e8"));
